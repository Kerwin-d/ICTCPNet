...
class Net:
    ...
    def set_loss_optimizer_scheduler(self):
        ...
        self.optim1 = optim.Adam(self.model.parameters(), lr=self.lr)
    ...
    def train_step(self, data):
        ...
        """cal loss"""
        self.csr_result, self.fusion_result = self.model(self.sdrRGB)
        self.loss1 = self.L1(self.csr_result[0], self.gtITP)                             #ITP空间loss联合优化
        self.loss2 = self.L1(self.fusion_result[0], self.gtITP)
        self.loss =  0.5 * self.loss1 + self.loss2       
        self.psnr_ITP = kornia.psnr_loss(self.fusion_result[0], self.gtITP, max_val=1)
        self.psnr_RGB = kornia.psnr_loss(self.fusion_result[1], self.gtRGB, max_val=1)
        """back"""
        self.loss.backward()
        for optim in self.optimizers:
            optim.step()
...
def train(opt):
    ...
    for epoch in range(opt.epoch_start, opt.epoch_end + 1):
        ...
        for i, data in enumerate(train_loader, 1):
            ...
            loss,psnr_ITP1,psnr_RGB1 = model.tensorboard()
            losses.append(loss)
            psnres_ITP.append(psnr_ITP1)
            psnres_RGB.append(psnr_RGB1)

            if i % 10 == 0:
                print('epoch:%d, batch:%d/%d, lr:%.7f,   '
                      'loss:%.6f, psnr_ITP1:%.2f, psnr_RGB1:%.2f, \n'
                      % (epoch, i, batch_num, lr, loss, psnr_ITP1, psnr_RGB1))
